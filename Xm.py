...
def parallel_distance_test_all(Hz, LZ, num_workers=None):
    ...
    return dz
def process_X(wid,mlx):
    filename =f"./data/hxhz162k0_{wid}_7.pkl"
    ...
    try:
        ...
        solutions = solve_gf2_linear_system(lx5_padded, Hx174_sortedall)
        threshold = Hx54_sortedall.shape[0]-1  # 获取阈值
        ...
        A, A1, B, B1 = process_L(H_new, lx5_padded_updated)
        ...
        print('Code parameters: n,k,d=',Hx174_sortedall.shape[1],k_d,min(dxa,dz))
        return (wid, Hx174_sortedall.shape[1],dz,dxb,dxa)
    except Exception as e:
        print(f"[ERROR] wid={wid} => {e}")
        return None
def process_XX(wid,w1,w2):
    filename = f"./data/hxhz162k0_{wid}_7.pkl"
    if not os.path.exists(filename):
        print(f"[SKIP] File not found: {filename}")
        return None
    ...
    try:
        ...
        Hx174_sortedall, xposition174_sorted, idx174 = sort_rows_by_position(Hx174_sorted, xposition174)
        Hx54_sortedall, xposition54_sorted, idx54 = sort_rows_by_position(Hx54_sorted, xposition54)
        lx51, removed_idx = insert_and_maintain_rank(Jxx, w1,w2)
        qcode174=css_code(Hx174_sortedall,Hz174_sortedall)
        print('Testing CSS code...')
        qcode174.test()
        print('Done')
        lx5_padded = pad_matrix_right_gf2(lx51, Hx174_sortedall.shape[1])
        solutions = solve_gf2_linear_system(lx5_padded, Hx174_sortedall)
        threshold = Hx54_sortedall.shape[0]-1  # 获取阈值
    ...
    for i in range(len(mlx)):
        for j in range(len(m1lx)):
            if i<j:
                results = []
                for wid in range(9, 16):
                    result = process_XX(wid,mlx[i],m1lx[j])
                    if result:
                        results.append(result)
                        _, _, _,  _, d = result
                        if d == dt0:
                            print(f"The target value d = {dt0} has been reached; exiting the loop early.")
                            break
                results_xx[u] = results
                u=u+1
    print("\n========= Summary X  =========")
    for i in range(len(resultsx)):
        print(f"X{mlx[i]}")
        print("wid\tn\tdz\tBefore opt dx\tAfter opt dx ")#(wid, Hx174_sortedall.shape[1],dz,dxb,dxa)
        for wid, n, dz,dxb,dxa in resultsx[i]:
            print(f"{wid}\t{n}\t{dz}\t{dxb}\t{dxa}")
    print("\n========= Summary XX =========")
    u=0
    for i in range(len(mlx)):
        for j in range(len(m1lx)):
            if i<j:
                print(f"X{mlx[i]} X{m1lx[j]}")
                print("wid\tn\tdz\tBefore opt dx\tAfter opt dx ")#(wid, Hx174_sortedall.shape[1],dz,dxb,dxa)
                for wid, n,  dz,dxb,dxa in results_xx[u]:
                    print(f"{wid}\t{n}\t{dz}\t{dxb}\t{dxa}")
                u=u+1