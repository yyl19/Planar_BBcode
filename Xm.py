...
def process_X(wid):
    ...
    try:
        ...
        V=Jz[len(B):, :]
        print("Before opt dz：")
        dz = Hz174_cleaned.shape[1]
        for i in range(U.shape[0]):
            w = distance_test(Hz174_cleaned,U[i,:])
        ...
        for i in range(len(B)):
            ...
            print('Logical qubit=',i,'Distance=',w)
            dx = min(dx,w)
        print('Code parameters: n,k,d=',Hz174_cleaned.shape[1],k_d,min(dz,dx))
        print("After opt dz：")
        U0=U
        U1=loop_update_U_W(U0, V, Hz174_cleaned, dt=dt0)
        d = Hz174_cleaned.shape[1]
        for i in range(U1.shape[0]):
            w = distance_test(Hz174_cleaned,U1[i,:])
            print('Logical qubit=',i,'Distance=',w)
            d = min(d,w)
        print('Code parameters: n,k,d=',Hx174_sortedall.shape[1],k_d,min(d,dx))
        return (wid, Hx174_sortedall.shape[1],dz,dx,d)
...
def process_XX(wid,w1,w2):
    ...
    try:
        ...
        for i in range(len(B)):
            ...
            print('Logical qubit=',i,'Distance=',w)
            dx = min(dx,w)
        print('Code parameters: n,k,d=',Hz174_cleaned.shape[1],k_d,min(d0,dx))
        print("After opt dz：")
        U0=U
        U1=loop_update_U_W(U0, V, Hz174_cleaned, dt=dt0)
        d = Hz174_cleaned.shape[1]
        ...
        print('Code parameters: n,k,d=',Hx174_sortedall.shape[1],k_d,min(d,dx))
        return (wid, Hx174_sortedall.shape[1],d0,dx,d)
...
if __name__ == "__main__":
    ...
    if mlx in m1lx:
        print("Xi and Xj must be different!!!")
        sys.exit()
    dt0=7
    ...
    for i in range(len(m1lx)):
        if mlx!=m1lx[i]:
            results = []
            for wid in range(9, 16):
                result = process_XX(wid,mlx,m1lx[i])
                if result:
                    ...
                    if d == dt0:
                        print(f"The target value dz = {dt0} has been reached; exiting the loop early.")
                        break
            results_xx[i] = results
    print("\n========= Summary X  =========")
    print(f"X{mlx}")
    print("wid\tn\tdx\tBefore opt dz\tAfter opt dz ")
    for wid, n,  d0,dz,d in resultsx:
        print(f"{wid}\t{n}\t{dz}\t{d0}\t{d}")
    print("\n========= Summary XX =========")
    for i in range(len(results_xx)):
        print(f"X{mlx} X{m1lx[i]}")
        print("wid\tn\tdx\tBefore opt dz\tAfter opt dz ")
        for wid, n,  d0,dz,d in results_xx[i]:
            print(f"{wid}\t{n}\t{dz}\t{d0}\t{d}")