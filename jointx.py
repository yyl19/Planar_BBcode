...
if __name__ == "__main__":
    ...
    dt0=7
    wid=25
    filename = f"./data/hxhz162l_{wid}_7.pkl"
    ...
    Hx174 = data["Hx"]
    Hz174 = data["Hz"]
    Jxx = np.loadtxt("./data/Jxx.txt", dtype=int)  
    Jzx = np.loadtxt("./data/Jzx.txt", dtype=int)
    xposition174 = data["xposition"]
    zposition174 = data["zposition"]
    edge_map174 = data["edge_map"]
    Hx174 = data["Hx"]
    ...
    lz5 = lz54.toarray()
    lx5 = lx54.toarray()
    M_inv = gf2_inverse_product(Jxx, lz5)
    lz5new = gf2_matmul(M_inv.T, lz5)
    qcode174=css_code(Hx174_sortedall,Hz174_sortedall)
    print('Testing CSS code...')
    qcode174.test()
    print('Done')
    GF = galois.GF(2)
    lx5_padded = pad_matrix_right_gf2(Jxx, Hx174_sortedall.shape[1])
    lx5_padded_left = pad_matrix_left_gf2(Jxx, Hx174_sortedall.shape[1])
    lx5_r_l=gf2_add(lx5_padded_left[0], lx5_padded[1])
    lx5_r_l = lx5_r_l.reshape(1, -1)
    solutions = solve_gf2_linear_system(lx5_r_l, Hx174_sortedall)
    threshold1 = Hx54_sortedall.shape[0]-1  # 获取阈值
    threshold2=Hx174_sortedall.shape[0]-Hx54_sortedall.shape[0]
    indices = get_zero_indices_between_thresholds(solutions[0],threshold1, threshold2)
    H_new, removed_cols = remove_rows_and_zero_columns(Hx174_sortedall, indices)
    lx5_padded_updated = remove_columns_from_matrix(lx5_padded, removed_cols)
    lx5_padded_left_updated = remove_columns_from_matrix(lx5_padded_left, removed_cols)
    Hz174_cleaned, removed_row_indices = remove_columns_and_zero_rows(Hz174_sortedall, removed_cols)
    lx5_r_l_updated=remove_columns_from_matrix(lx5_r_l, removed_cols)
    qcode174_d=css_code(H_new,Hz174_cleaned)
    print('Testing CSS code...')
    qcode174_d.test()
    print('Done')
    ...
    A0, A01, B0, B01 = process_L(H_new, lx5_r_l_updated)
    lx5_paddedstack = np.vstack([lx5_padded_updated, lx5_padded_left_updated])
    A, A1, B, B1 = process_L(H_new, lx5_paddedstack)
    Jx, indices,C = construct_Jx(B, lx17_d, H_new)
    GT=gf2_inverse_product(Jx, lz17_d)
    Jz=gf2_matmul(GT.T, lz17_d)
    U = Jz[:len(B), :]
    V=Jz[len(B):, :]
    print("Before opt dx:")
    dxb = parallel_distance_test_all(Hz174_cleaned, U, num_workers=U.shape[0]+1) 
    print("dz:")
    dz = parallel_distance_test_all(H_new, np.array(B), num_workers=len(B)+1) 
    print('Code parameters: n,k,d=',Hz174_cleaned.shape[1],k_d,min(dz,dxb))
    print("After opt dx:")
    U0=U
    U1=loop_update_U_W_parallel(U0, V, Hz174_cleaned, dt=dt0, num_workers=U.shape[0]+1)
    dxa = parallel_distance_test_all(Hz174_cleaned, U1, num_workers=U1.shape[0]+1)
    print('Code parameters: n,k,d=',Hx174_sortedall.shape[1],k_d,min(dz,dxa))
    print("\n========= Summary of Code Parameters =========")
    print(f"X{0} tensor X{2}")
    print("wid\tn\tdz\tbefore opt dx\tafter opt dx ")
    print(f"{wid}\t{Hx174_sortedall.shape[1]}\t{dz}\t{dxb}\t{dxa}")